...
/*
TODO: The simulate_readout function in the example would be replaced by network-listening code assuming data coming via TCP bridge
*/
...
int main() {
    ...
    */
    std::thread t1(simulate_readout, std::ref(buffer), SubsystemId::Tracker, 1, tracker_payload);
    std::this_thread::sleep_for(std::chrono::milliseconds(50));
    std::thread t2(simulate_readout, std::ref(buffer), SubsystemId::Hcal, 1, hcal_payload);
    t1.join();
    t2.join();
