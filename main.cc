...
PhysicsEventData assemble_payload(unsigned int event_id, const std::vector<DataFragment>& fragments) {
    ...
    for (const auto& fragment : fragments) {
        ...
        if (fragment.header.subsystem_id == SubsystemId::Tracker) {
            ...
            event_data.tracker_info.hits.push_back({1.0, 2.0, 3.0, 1});
            ...
            event_data.hcal_info.cells.push_back({0.5, 1, 1});
        }
    }
    return event_data;
}
...
int main() {
    ...
    if (buffer.try_build_event(event_id_to_build)) {
        ...
        std::cout << "Tracker Data Size: " << combined_event->get_payload().tracker_info.hits.size() * sizeof(TrackerHit) << " bytes" << std::endl;
        std::cout << "HCAL Data Size: " << combined_event->get_payload().hcal_info.cells.size() * sizeof(HcalCell) << " bytes" << std::endl;
    } else {
