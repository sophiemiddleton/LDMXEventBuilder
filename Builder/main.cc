...
PhysicsEventData assemble_payload(const std::vector<DataFragment>& fragments) {
    ...
    for (const auto& fragment : fragments) {
        ...
        if (fragment.header.subsystem_id == 0) {
            ...
            TrkData current_trk_data = deserialize_tracker_data();
            if (!has_tracker) {
    ...
    std::thread simulation_thread([&]() {
        ...
        for (unsigned int i = 1; i <= 5; ++i) {
            ...
            for (int h = 0; h < num_ecal_fragments; ++h) {
                ...
                simulate_tcp_client(1, i, ecal_frag_timestamp, serialize_ecal_data(ecal_data), port);
                std::this_thread::sleep_for(std::chrono::milliseconds(20));
            }
            ...
            for (int h = 0; h < num_hcal_fragments; ++h) {
              ...
              for (int hit = 0; hit < num_bar_hits; ++hit) {
                ...
                simulate_tcp_client(2, i, hcal_frag_timestamp, serialize_hcal_data(hcal_data), port);
                std::this_thread::sleep_for(std::chrono::milliseconds(20));
            }
